...
- [x] Opus + Sonnet review; findings fixed
  - **No correctness findings from either reviewer.** `JsonElement` copy semantics on the deserialize/reframe path confirmed independently by both; `ArrayPool` rent/return discipline verified per code path (payload lease returned on every exit incl. exceptions; reader buffer returned on dispose); 6d in-place metadata ownership-transfer adversarially traced clean (freshly-deserialized dict, projected once, ownership keys stamped last so they always win).
  - **Coverage added** (445 default + 8 ExternalTooling green): `HeaderFramingTests` — `HeaderLargerThanInitialBuffer_CompactsAndGrows` (exercises the previously-dead `EnsureReadSpace` grow branch via the small-buffer ctor), `CancellationDuringPayloadRead_ThrowsAndReturnsBuffers` (deterministic mid-payload cancel; asserts a fresh reader still works, i.e. no pool corruption), `NegativeContentLength_Throws`, `PlusSignContentLength_ParsesLikeUtf8Parser`, `FrameLease_Default_DisposeIsNoOp`. `AnalysisDeltaApplierTests` — `MutableMetadataWithConflictingOwnershipKey_*`, `NullMetadata_ProducesExactlyTheTwoOwnershipEntries`, `NonDictionaryReadOnlyMetadata_DefensivelyCopied*` (all three `BuildOwnedMetadata` branches).
  - **Deliberate narrowings documented** (code comments, no behavior change): `Utf8Parser` vs former `int.TryParse` — verified it still accepts a leading '+' ("+5" → 5), so the two agree; `TrimAsciiSpace` is ASCII space/tab only vs former Unicode `string.Trim()` (headers are ASCII by spec). The grow branch is annotated defensive-only (DefaultInitialBufferSize 8192 > MaxHeaderBytes 4096).
