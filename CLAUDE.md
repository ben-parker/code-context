...
### Parser extensibility (`ILanguageParser`)
...
`ILanguageParser` is the **legacy in-process seam**, kept for tests and future adapters — no production parser uses it anymore. New language support is added as an out-of-process worker: an executable speaking the parser protocol plus a `worker-manifest.json` under `workers/<name>/` (see `src/CodeContext.CSharp.Worker` and `src/CodeContext.TypeScript.Worker` as the reference implementations and `docs/language-worker-architecture-plan.md` for the rules).

### Worker-owned languages vs. single-file fallback

File extensions claimed by a discovered worker manifest route through `ILanguageWorkerService`: a change batch becomes one `workspace/applyChanges` request (scans become `workspace/index`), the worker reparses only the changed files against its cached compilation state, and streams back a complete workspace replacement as `analysis/delta` chunks. `AnalysisDeltaApplier` buffers the chunks and commits them atomically as a new **generation** through `IGenerationalGraphStore`: the commit replaces only that parser/workspace's facts (ownership via `parserId`/`workspaceId` metadata), readers keep seeing the previous complete snapshot until the swap, and stale generations are rejected. On the opt-in Kuzu backend, worker deltas fall back to `JsonReconcileDeltaSink` → `ICodeGraphRepository.ReconcileAndPruneAsync` (whole-graph JSON replacement — JSON is the process boundary to `kuzu_api.py`, and the clobber of other parsers' facts is the pre-existing Kuzu limitation).

Extensions without a worker go through `ProcessFileChangeOldWayAsync`, which parses one file at a time with an in-process `ILanguageParser` and updates just that file's nodes/edges. No production parser uses this path anymore (C# and TypeScript are both workers); it remains as the seam for tests and future in-process adapters.
...
`ContextService` is the engine behind both `/api/context/complete` and the MCP `GetContext` tool. Both surfaces default to the compact view: exact-first name resolution, depth 1, tests/related items/metrics/content off, at most five ambiguous summaries, and at most ten entries per relationship list. Ambiguous queries are not expanded unless `expandAmbiguous=true`; explicit expansion remains capped. `view=full` selects the legacy comprehensive response. Given an `identifier` (name or an absolute/repository-relative source path), the service assembles deduplicated `uses`/`usedBy` graph traversal up to the requested depth (`0` skips relationships), file-level `Dependencies`/`DependedBy` (graph edges plus C# using-namespace hints), optional same-file/same-namespace `RelatedItems`, attributed/fallback-detected tests, and optional heuristic metrics. C# workers emit `REFERENCES` edges for source-defined concrete types used in fields, properties, parameters, return types, locals, and object creation, so concrete-type blast-radius queries are not limited to inheritance.
