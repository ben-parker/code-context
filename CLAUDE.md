...
The host is spawned once per repository, so cold start and per-query cost both matter. The .NET 10 upgrade (see `DOTNET10_UPGRADE_TASKS.md` for the full phase log + final benchmark table) settled the shape:

- **Native AOT host + R2R worker** — single native binary, no JIT warm-up on the per-repo-spawn path (`AOT_COMPATIBILITY.md`).
- **Adjacency indexes** — `InMemoryDatabase.GetAdjacency()` exposes a `GraphAdjacency` (FrozenDictionary `EdgesBySource`/`EdgesByTarget`/`NodesByFilePath` + cached node/edge snapshots) rebuilt lazily behind a **version-stamped cache** (one immutable `CachedAdjacency` holder in a `volatile` field, modeled on `GetStatistics()`; reads see only committed generations). This turns every `ContextService` hop's edge lookup from an O(E) table scan into an O(degree) lookup — the dominant query win.
- **Pooled protocol framing** — `HeaderFraming`/`FrameReader` (`CodeContext.Parser.Protocol`) reads frames into `ArrayPool` buffers with a span header parse, no byte-at-a-time reads; commit-path and file-enumeration allocations were trimmed in the same pass.
- **Benchmark methodology** lives in `scripts/measure-startup.ps1` (time-to-listening + time-to-indexed) and the Benchmarks section of `DOTNET10_UPGRADE_TASKS.md` (cold `--version`, query p50 against `%TEMP%\cc-repo-copy`, publish payload size, alloc-rate via `dotnet-counters`). Re-measure on an AOT publish from `scripts/verify-publish.ps1`, not a JIT `dotnet build`.
